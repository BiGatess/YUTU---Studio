...
import WebtoonLogo from '../../../assets/images/logo.jpg';
...
const CustomLanguageSelector = () => {
  ...
  const handleSelect = (lang) => { setSelectedLang(lang); setIsOpen(false); };
  return (
    <div className={styles.languageSelector}>
      {isOpen && ( <ul className={styles.languageOptions}> {options.map(option => ( <li key={option} onClick={() => handleSelect(option)} className={option === selectedLang ? styles.selected : ''}> <span>{option}</span> {option === selectedLang && <span className={styles.checkmark}>✓</span>} </li> ))} </ul> )}
      <button type="button" className={styles.languageButton} onClick={handleToggle}> {selectedLang} <span className={styles.arrow}>{isOpen ? '▲' : '▼'}</span> </button>
    </div>
  );
};
  ...
  return (
    <footer className={styles.footerContainer}>
      <div className={styles.contentWrapper}>
        ...
        {/* Hàng 2: Nav Links + Language Selector */}
        ...
        <div className={styles.logoContainer}>
          <img src={WebtoonLogo} alt="Yutu Studio Logo" />
        </div>
